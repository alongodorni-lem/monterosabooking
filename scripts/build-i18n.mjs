...
function injectScripts(html, lang) {
  ...
  out = out.replace(
    ...
    `<script src="${prefix}js/i18n.js?v=7"></script>\n  <script src="${prefix}js/partials.js?v=14"></script>`
  );
  out = out.replace(
    /(\.\.\/)?js\/esperienze-list\.js\?v=\d+/g,
    `${prefix}js/esperienze-list.js?v=12`
  );
  out = out.replace(
    /(\.\.\/)?js\/embed-widget\.js\?v=\d+/g,
    `${prefix}js/embed-widget.js?v=3`
  );
  out = out.replace(/(\.\.\/)?js\/main\.js/g, `${prefix}js/main.js`);
  out = out.replace(/(\.\.\/)?css\/style\.css\?v=\d+/g, `${prefix}css/style.css?v=16`);
  return out;
}
...
function patchItalian(srcHtml, file) {
  ...
  if (!html.includes("js/i18n.js")) {
    html = html.replace(
      /<script src="js\/partials\.js\?v=\d+"><\/script>/g,
      '<script src="js/i18n.js?v=7"></script>\n  <script src="js/partials.js?v=14"></script>'
    );
  } else {
    html = html.replace(/js\/i18n\.js\?v=\d+/g, "js/i18n.js?v=7");
    html = html.replace(/js\/partials\.js\?v=\d+/g, "js/partials.js?v=14");
  }
  html = html.replace(/js\/esperienze-list\.js\?v=\d+/g, "js/esperienze-list.js?v=12");
  html = html.replace(/js\/embed-widget\.js\?v=\d+/g, "js/embed-widget.js?v=3");
  html = html.replace(/css\/style\.css\?v=\d+/g, "css/style.css?v=16");
